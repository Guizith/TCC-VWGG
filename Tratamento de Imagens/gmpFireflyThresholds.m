...
%
%
function bests = gmpFireflyThresholds(histograma,thresholds,NFireflies,iter,METHOD)
...
switch METHOD
    ...
    case {'TE'}
      for k=1:n  
         zn(k)= psrAvaliacaoTsallis(histograma, 0.1, fireflies(k,:));  
      end
    otherwise
        ...
        if Lightn(j)>Lightn(i), % Brighter and more attractive
            ...
            if(i~=ni)
                for k=1:size(elementos,2) 
                    ...
                    %elementos(i,k)=(beta)*elementos(i,k)+(1-beta)*(elementos(j,k))+vet(1,k);                       
